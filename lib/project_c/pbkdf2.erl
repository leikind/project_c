...
-export([compare_secure/2, pbkdf2/4, pbkdf2/5,
	 to_hex/1]).
...
-type digest_func_info() :: md4 | md5 | ripemd160 |
			    sha | sha224 | sha256 | sha384 | sha512.

-type mac_func_info() :: {hmac, digest_func_info()} |
			 digest_func_info().
...
-spec pbkdf2(MacFunc, Password, Salt,
	     Iterations) -> {ok, Key} |
			    {error, derived_key_too_long} when MacFunc ::
								   mac_func_info(),
							       Password ::
								   binary(),
							       Salt :: binary(),
...
pbkdf2(MacFunc, Password, Salt, Iterations) ->
    ...
    DerivedLength = byte_size(MacFunc1(<<"test key">>,
				       <<"test data">>)),
    Bin = pbkdf2(MacFunc1, Password, Salt, Iterations,
		 DerivedLength, 1, []),
    {ok, Bin}.

%----------------------------------------------------------------------------------------------------------------------

-spec pbkdf2(MacFunc, Password, Salt, Iterations,
	     DerivedLength) -> {ok, Key} |
			       {error, derived_key_too_long} when MacFunc ::
								      mac_func_info(),
								  Password ::
								      binary(),
...
pbkdf2(_MacFunc, _Password, _Salt, _Iterations,
       DerivedLength)
...
-spec to_hex(Data) -> HexData when Data :: binary() |
					   list(),
...
to_hex([]) -> [];
to_hex([Char | Rest]) ->
    [to_hex_digit(Char div 16), to_hex_digit(Char rem 16)
     | to_hex(Rest)].
...
-spec pbkdf2(MacFunc, Password, Salt, Iterations,
	     DerivedLength, BlockIndex, Acc) -> Key when MacFunc ::
							     fun((binary(),
								  binary()) -> binary()),
							 Password :: binary(),
							 Salt :: binary(),
...
pbkdf2(MacFunc, Password, Salt, Iterations,
    ...
    case iolist_size(Acc) > DerivedLength of
	  ...
	  Bin;
      false ->
	  Block = pbkdf2(MacFunc, Password, Salt, Iterations,
			 BlockIndex, 1, <<>>, <<>>),
	  pbkdf2(MacFunc, Password, Salt, Iterations,
		 DerivedLength, BlockIndex + 1, [Block | Acc])
    end.

%----------------------------------------------------------------------------------------------------------------------

-spec pbkdf2(MacFunc, Password, Salt, Iterations,
	     BlockIndex, Iteration, Prev, Acc) -> Key when MacFunc ::
							       fun((binary(),
								    binary()) -> binary()),
							   Password :: binary(),
							   Salt :: binary(),
...
pbkdf2(_MacFunc, _Password, _Salt, Iterations,
       _BlockIndex, Iteration, _Prev, Acc)
    when Iteration > Iterations ->
    Acc;
pbkdf2(MacFunc, Password, Salt, Iterations, BlockIndex,
       1, _Prev, _Acc) ->
    InitialBlock = MacFunc(Password,
			   <<Salt/binary, BlockIndex:32/integer>>),
    pbkdf2(MacFunc, Password, Salt, Iterations, BlockIndex,
	   2, InitialBlock, InitialBlock);
pbkdf2(MacFunc, Password, Salt, Iterations, BlockIndex,
       Iteration, Prev, Acc) ->
    Next = MacFunc(Password, Prev),
    pbkdf2(MacFunc, Password, Salt, Iterations, BlockIndex,
	   Iteration + 1, Next, crypto:exor(Next, Acc)).
...
resolve_mac_func(ripemd160) ->
    resolve_mac_func({hmac, ripemd160});
resolve_mac_func(sha) -> resolve_mac_func({hmac, sha});
resolve_mac_func(sha224) ->
    resolve_mac_func({hmac, sha224});
resolve_mac_func(sha256) ->
    resolve_mac_func({hmac, sha256});
resolve_mac_func(sha384) ->
    resolve_mac_func({hmac, sha384});
resolve_mac_func(sha512) ->
    resolve_mac_func({hmac, sha512}).

%----------------------------------------------------------------------------------------------------------------------

%% Compare two strings or binaries for equality without short-circuits to avoid timing attacks.

-spec compare_secure(First,
		     Second) -> boolean() when First :: binary() | string(),
...
-spec compare_secure(First, Second,
		     Accum) -> boolean() when First :: string(),
					      Second :: string(),
					      Accum :: integer().

compare_secure([X | RestX], [Y | RestY], Result) ->
    compare_secure(RestX, RestY, X bxor Y bor Result);
compare_secure([], [], Result) -> Result == 0.

%----------------------------------------------------------------------------------------------------------------------

-spec to_hex_digit(Nyble) -> hex_char() when Nyble ::
						 0..15.

to_hex_digit(N) when N < 10 -> $0 + N;
to_hex_digit(N) -> $a + N - 10.
